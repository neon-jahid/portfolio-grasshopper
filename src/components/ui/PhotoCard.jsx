 ...
 *
 * The whole card is the button that opens the viewer. `width` and `height`
 * come from the data file and are used only as a ratio: the space is reserved
 * before the file loads, so the masonry never reflows mid-scroll.
 *
...
export function PhotoCard({ photo, onOpen, className }) {
  return (
    <button
      ...
      className={cn(
        'group relative block w-full cursor-zoom-in overflow-hidden rounded-2xl',
